...
def inference(model, test_loader, output_dir):
    ...
    for i, (image, meta) in enumerate(test_loader):
        ...
        # get_cosine_map(embed = output_dict["embed"], conf = output_dict["fy_preds"][:,0,:,:],imgfname=meta['image_id'][idx])
        # vis_show_1 = output_dict["embed"][0][0].detach().cpu().numpy()*255
        # cv2.imwrite("vis/cosine_map/" + meta['image_id'][idx][:-4]+"_1.jpg", vis_show_1)
        # vis_show_2 = output_dict["embed"][0][1].detach().cpu().numpy()*255
        # cv2.imwrite("vis/cosine_map/" + meta['image_id'][idx][:-4]+"_2.jpg", vis_show_2)
        ...
        if cfg.exp_name == "Icdar2015":
            ...
            fname = "res_" + meta['image_id'][idx].split(".")[0]+".txt"
            data_transfer_TD500HUST(contours, os.path.join(output_dir, fname))
            ...
            for j in range(len(contours)):
                ...
                if S < 0:
                    ...
                    # art_res['points'] = contours[j].tolist()
                    continue
            ...
            # tmp = []
            # for con in contours:
            #     epsilon = 0.002 * cv2.arcLength(con, True)
            #     con = cv2.approxPolyDP(con, epsilon, True)
            #     tmp.append(con)
            # tmp = np.array(tmp)
